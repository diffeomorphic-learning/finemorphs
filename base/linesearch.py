...
def line_search_armijo(opt, pk, gfk=None, old_fval=None, c1=0.001,
...
#    logging.info('starting backtrack')
    ...
    if it == maxiter:
        logging.warning(f'Backtracking: maximum number of iterations attained {fval:.5f}--{old_fval:.5f}')
        t = None

    # logging.info(f'Objective after line search {t_init:5f} {fval:.4f}, {opt.obj:.4f}, {opt.objTry:.4f}')
    return t, fc[0], gc[0], fval, old_fval, gval[0]

def line_search_weak_wolfe(opt, pk, gfk=None, old_fval=None,
                           old_old_fval=None, c1=0.001, c2=0.9, amax=None,
                           maxiter=100, euclidean=True, t_init = 1.):
#    logging.info('Starting weak wolfe line search')
    fc = [0]
    gc = [0]
    gval = [None]
    gval_alpha = [None]
